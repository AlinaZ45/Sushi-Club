...
renderAll()}
function renderAll(){renderStats();renderRequests();renderTables();renderSlots();renderTableInventory();renderTableManagement()}
...
function statusButtons(r){
  const table='<button onclick="window.staffOpenAssign('+r.id+')">'+t('chooseTable')+'</button>';
...
function tableCard(tbl,time){if(!time||time==='all')return '<div class="table-card"><b>'+esc(tbl.table_code)+'</b><small>'+t('seatCount',{n:tbl.seats})+'</small><small>'+t('selectTime')+'</small></div>';const occ=occupantForTable(tbl.id,time);const blockedSlot=isBlocked(time);return '<div class="table-card '+(occ?'used':'free')+(blockedSlot?' blocked':'')+'"><b>'+esc(tbl.table_code)+'</b><small>'+t('seatCount',{n:tbl.seats})+'</small><small>'+(blockedSlot?t('slotBlocked'):occ?esc(occ.name)+' · '+occ.guests+'p':t('available'))+'</small></div>'}
...
window.staffSetStatus=async function(id,status){
  const s=await staffSession();if(!s)return alert(t('signAgain'));
  const r=bookingById(id);if(!r)return;
  let staffNote=r.staffNote||'';

  if(status==='declined'){
    const reason=prompt(t('declineReasonPrompt'),staffNote||'');
    if(reason===null)return;
    staffNote=reason.trim();
    if(!staffNote)return alert(t('declineReasonRequired'));
  }

  if(status==='confirmed'&&r.status==='pending'){
    const own=assignmentIds(r.id);
    if(own.length){
      if(assignedSeats(r.id)<r.guests)return alert(t('assignedSeatsError'));
      for(const tableId of own){
        const occ=occupantForTable(tableId,r.time,r.id);
        if(occ&&ACTIVE_FOR_CAPACITY.includes(occ.status))return alert(t('assignedUsedError'));
      }
    }
    try{
      const j=await apiGet({action:'availability',date:r.date,party:r.guests});
      const slot=(j.slots||[]).find(x=>x.time===r.time);
      if(slot&&!slot.available)return alert(t('slotCapacityError'));
    }catch(e){return alert(t('availabilityError'))}
  }

  const patch={status};
  if(status==='declined')patch.staff_note=staffNote;
  const {error}=await sb.from('reservations').update(patch).eq('id',id);
  if(error)return alert(error.message);

  if(['declined','cancelled','completed','no_show'].includes(status)){
    const {error:de}=await sb.from('table_assignments').delete().eq('reservation_id',id);
    if(de)return alert(de.message);
  }

  await addAudit(id,'STAFF_STATUS',r.status,status,status==='declined'?{reason:staffNote}:{});
  await loadData();
}
...
function tableRpcError(error){
  const m=String(error?.message||error||'');
  if(m.includes('INVALID_TABLE_NAME'))return t('tableNameHint');
  if(m.includes('INVALID_TABLE_AREA')||m.includes('INVALID_TABLE_SEATS'))return t('invalidAreaSeats');
  if(m.includes('INVALID_TABLE_QUANTITY'))return t('invalidQuantity');
  if(m.includes('TABLE_NAME_EXISTS:'))return t('tableCodeExists')+' '+m.split('TABLE_NAME_EXISTS:')[1];
  if(m.includes('TABLE_HAS_HISTORY'))return t('deleteHistory');
  if(m.includes('SELECT_TABLES_FIRST'))return t('selectTablesToRemove');
  return m;
}

async function addTable(){
  const s=await staffSession();if(!s)return alert(t('signAgain'));
  const name=$('newTableCode').value.trim(),area=$('newTableArea').value,seats=Number($('newTableSeats').value),quantity=Number($('newTableQuantity').value);
  const msg=$('tableAdminMessage');msg.textContent='';msg.className='message';
  if(name.length<1||name.length>30){msg.textContent=t('tableNameHint');msg.classList.add('err');return}
  if(!['inside','terrace'].includes(area)||!Number.isInteger(seats)||seats<1||seats>20){msg.textContent=t('invalidAreaSeats');msg.classList.add('err');return}
  if(!Number.isInteger(quantity)||quantity<1||quantity>20){msg.textContent=t('invalidQuantity');msg.classList.add('err');return}
  const {error}=await sb.rpc('sushi_table_admin',{p_action:'add',p_id:null,p_name:name,p_area:area,p_seats:seats,p_quantity:quantity,p_ids:null});
  if(error){msg.textContent=tableRpcError(error);msg.classList.add('err');return}
  $('newTableCode').value='';$('newTableSeats').value='2';$('newTableQuantity').value='1';
  msg.textContent=t('tableAddedMany',{count:quantity});msg.classList.add('ok');
  selectedInventoryTables.clear();
  await loadData()
}

function renderTableInventory(){
  const host=$('tableInventory'),summary=$('tableInventorySummary'),sel=$('tableSelectionCount');
  if(!host||!summary)return;
  const ordered=[...allTables].sort((a,b)=>{
    const areaA=a.area==='inside'?0:1,areaB=b.area==='inside'?0:1;
    return areaA-areaB||(Number(a.sort_order)||0)-(Number(b.sort_order)||0)||String(a.table_code).localeCompare(String(b.table_code));
  });
  const validIds=new Set(ordered.map(x=>Number(x.id)));
  selectedInventoryTables=new Set([...selectedInventoryTables].filter(id=>validIds.has(Number(id))));
  const totalSeats=ordered.filter(x=>x.active!==false).reduce((n,x)=>n+Number(x.seats||0),0);
  summary.textContent=t('tablesSummary',{tables:ordered.length,seats:totalSeats});
  if(sel)sel.textContent=selectedInventoryTables.size?t('selectedTablesCount',{count:selectedInventoryTables.size}):t('selectTablesToRemove');
  host.innerHTML=ordered.length?ordered.map(tbl=>{
    const selected=selectedInventoryTables.has(Number(tbl.id));
    return '<div class="inventory-card '+(tbl.active===false?'inactive ':'')+(selected?'selected-for-remove':'')+'" onclick="window.staffToggleInventoryTable('+Number(tbl.id)+')">'+
      '<div class="inventory-check">'+(selected?'✓':'')+'</div>'+
      '<div class="inventory-code">'+esc(tbl.table_code)+'</div>'+
      '<div class="inventory-meta">'+esc(tbl.area==='inside'?t('inside'):t('terrace'))+' · '+t('seatCount',{n:Number(tbl.seats||0)})+'</div>'+
      '<div class="inventory-state">'+(tbl.active===false?t('inactive'):t('active'))+'</div>'+
    '</div>'
  }).join(''):'<p class="note">'+t('noTables')+'</p>';
}
window.staffToggleInventoryTable=function(id){
  id=Number(id);
  if(selectedInventoryTables.has(id))selectedInventoryTables.delete(id);else selectedInventoryTables.add(id);
  renderTableInventory();
}
async function removeSelectedTables(){
  const s=await staffSession();if(!s)return alert(t('signAgain'));
  const ids=[...selectedInventoryTables].map(Number);
  if(!ids.length)return tableAdminMessage(t('selectTablesToRemove'));
  if(!confirm(t('bulkRemoveConfirm',{count:ids.length})))return;
  const {data,error}=await sb.rpc('sushi_table_admin',{p_action:'remove_many',p_id:null,p_name:null,p_area:null,p_seats:null,p_quantity:1,p_ids:ids});
  if(error)return tableAdminMessage(tableRpcError(error));
  selectedInventoryTables.clear();
  tableAdminMessage(t('bulkRemoveDone',{deleted:data?.deleted??0,disabled:data?.disabled??0}),true);
  await loadData();
}
...
function renderTableManagement(){
  ...
  host.innerHTML=allTables.length?allTables.map(tbl=>'<div class="table-manage-row '+(tbl.active===false?'inactive':'')+'" data-id="'+Number(tbl.id)+'">'+
    '<div><label>'+t('tableName')+'</label><input id="mtCode'+Number(tbl.id)+'" value="'+esc(tbl.table_code)+'" maxlength="30"><div class="table-manage-state">'+(tbl.active===false?t('inactive'):t('active'))+'</div></div>'+
...
window.staffSaveTable=async function(id){
  const s=await staffSession();if(!s)return alert(t('signAgain'));
  const tbl=allTables.find(x=>Number(x.id)===Number(id));if(!tbl)return;
  const name=$('mtCode'+id).value.trim(),area=$('mtArea'+id).value,seats=Number($('mtSeats'+id).value);
  if(name.length<1||name.length>30)return tableAdminMessage(t('tableNameHint'));
  if(!['inside','terrace'].includes(area)||!Number.isInteger(seats)||seats<1||seats>20)return tableAdminMessage(t('invalidAreaSeats'));
  const {error}=await sb.rpc('sushi_table_admin',{p_action:'update',p_id:id,p_name:name,p_area:area,p_seats:seats,p_quantity:1,p_ids:null});
  if(error)return tableAdminMessage(tableRpcError(error));
...
window.staffToggleTableActive=async function(id){
  ...
  const {error}=await sb.rpc('sushi_table_admin',{p_action:'toggle',p_id:id,p_name:null,p_area:null,p_seats:null,p_quantity:1,p_ids:null});
  if(error)return tableAdminMessage(tableRpcError(error));
  tableAdminMessage(tbl.active===false?t('tableActivated'):t('tableDisabled'),true);await loadData();
}
window.staffDeleteTable=async function(id){
  const s=await staffSession();if(!s)return alert(t('signAgain'));
  const tbl=allTables.find(x=>Number(x.id)===Number(id));if(!tbl)return;
  if(!confirm(t('deleteConfirm',{code:tbl.table_code})))return;
  const {error}=await sb.rpc('sushi_table_admin',{p_action:'delete',p_id:id,p_name:null,p_area:null,p_seats:null,p_quantity:1,p_ids:null});
  if(error)return tableAdminMessage(tableRpcError(error));
