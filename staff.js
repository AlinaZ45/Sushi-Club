...
function assignedCodes(reservationId){const x=assignedTables(reservationId);return x.length?x.map(t=>t.table_code).join(' + '):'Not assigned'}
...
function statusButtons(r){
  const table='<button onclick="window.staffOpenAssign('+r.id+')">TABLES</button>';
  if(r.status==='pending')return '<div class="actions three"><button class="primary" onclick="window.staffSetStatus('+r.id+',\'confirmed\')">CONFIRM</button><button class="danger" onclick="window.staffSetStatus('+r.id+',\'declined\')">DECLINE</button>'+table+'</div>';
  if(['confirmed','reconfirmed'].includes(r.status))return '<div class="actions three">'+table+'<button class="primary" onclick="window.staffSetStatus('+r.id+',\'seated\')">SEATED</button><button class="danger" onclick="window.staffSetStatus('+r.id+',\'no_show\')">NO-SHOW</button></div>';
  if(r.status==='seated')return '<div class="actions">'+table+'<button class="primary" onclick="window.staffSetStatus('+r.id+',\'completed\')">COMPLETE</button></div>';
  return ''
}
...
function tableCard(t,time){if(!time||time==='all')return '<div class="table-card"><b>'+esc(t.table_code)+'</b><small>'+t.seats+' seats</small><small>Select a time</small></div>';const occ=occupantForTable(t.id,time);const blockedSlot=isBlocked(time);return '<div class="table-card '+(occ?'used':'free')+(blockedSlot?' blocked':'')+'"><b>'+esc(t.table_code)+'</b><small>'+t.seats+' seats</small><small>'+(blockedSlot?'Slot blocked':occ?esc(occ.name)+' · '+occ.guests+'p':'Available')+'</small></div>'}
function renderTables(){const time=$('timeFilter').value,inside=tables.filter(t=>t.area==='inside'),terrace=tables.filter(t=>t.area==='terrace');let html='';if(time==='all')html+='<p class="note" style="margin:0 0 12px">Choose a time above to see the live table plan.</p>';else if(isBlocked(time))html+='<p class="note" style="margin:0 0 12px">'+esc(time)+' is blocked for new reservations.</p>';html+='<div class="area-title">INSIDE</div><div class="table-grid">'+inside.map(t=>tableCard(t,time)).join('')+'</div><div class="area-title" style="margin-top:17px">TERRACE</div><div class="table-grid">'+terrace.map(t=>tableCard(t,time)).join('')+'</div>';$('tablesView').innerHTML=html}
...
function renderSlots(){const cap=tables.reduce((n,t)=>n+Number(t.seats||0),0);$('slots').innerHTML=TIMES.map(t=>'<div class="slot"><b>'+t+'</b><small>'+(isBlocked(t)?'Closed for new reservations':confirmedGuests(t)+' / '+cap+' confirmed guests · '+usedTableCount(t)+' tables assigned')+'</small><button class="'+(isBlocked(t)?'open':'')+'" onclick="window.staffToggleSlot(\''+t+'\')">'+(isBlocked(t)?'OPEN':'BLOCK')+'</button></div>').join('')}
...
window.staffSetStatus=async function(id,status){const s=await staffSession();if(!s)return alert('Please sign in again.');const r=bookingById(id);if(!r)return;if(status==='confirmed'&&r.status==='pending'){const own=assignmentIds(r.id);if(own.length){if(assignedSeats(r.id)<r.guests)return alert('Assigned tables do not have enough seats for this reservation.');for(const tableId of own){const occ=occupantForTable(tableId,r.time,r.id);if(occ&&ACTIVE_FOR_CAPACITY.includes(occ.status))return alert('One of the assigned tables is already used by another confirmed reservation.')}}try{const j=await apiGet({action:'availability',date:r.date,party:r.guests});const slot=(j.slots||[]).find(x=>x.time===r.time);if(slot&&!slot.available)return alert('This time no longer has enough table capacity. Please choose another time or free a table first.')}catch(e){return alert('Availability could not be verified. Please try again.')}}const {error}=await sb.from('reservations').update({status}).eq('id',id);if(error)return alert(error.message);if(['declined','cancelled','completed','no_show'].includes(status)){const {error:de}=await sb.from('table_assignments').delete().eq('reservation_id',id);if(de)return alert(de.message)}await addAudit(id,'STAFF_STATUS',r.status,status,{});await loadData()}
window.staffToggleSlot=async function(time){const s=await staffSession();if(!s)return alert('Please sign in again.');const date=$('date').value,hit=blocked.find(x=>String(x.reservation_time||x.block_time||'').slice(0,5)===time);let error;if(hit)({error}=await sb.from('blocked_slots').delete().eq('id',hit.id));else({error}=await sb.from('blocked_slots').insert({reservation_date:date,reservation_time:time+':00',block_date:date,block_time:time+':00',active:true,created_by:s.session.user.id,reason:'Staff block'}));if(error)return alert(error.message);await loadData()}
function modalTableCard(t,r){const occ=occupantForTable(t.id,r.time,r.id),selected=selectedTables.has(Number(t.id)),disabled=!!occ;return '<button type="button" class="table-card selectable '+(selected?'selected ':'')+(disabled?'disabled':'')+'" '+(disabled?'disabled':'onclick="window.staffToggleTable('+Number(t.id)+')"')+'><b>'+esc(t.table_code)+'</b><small>'+t.seats+' seats</small><small>'+(occ?'Used by '+esc(occ.name):selected?'Selected':'Available')+'</small></button>'}
function renderAssignModal(){const r=bookingById(assigningId);if(!r)return;const inside=tables.filter(t=>t.area==='inside'),terrace=tables.filter(t=>t.area==='terrace'),selected=[...selectedTables].map(tableById).filter(Boolean),seat=selected.reduce((n,t)=>n+Number(t.seats||0),0);$('assignTitle').textContent=r.time+' · '+r.name;$('assignMeta').textContent=r.guests+' guest(s) · '+r.code;$('assignGrid').innerHTML='<div class="area-title">INSIDE</div><div class="table-grid">'+inside.map(t=>modalTableCard(t,r)).join('')+'</div><div class="area-title" style="margin-top:17px">TERRACE</div><div class="table-grid">'+terrace.map(t=>modalTableCard(t,r)).join('')+'</div>';$('assignSeatSummary').innerHTML='<b>'+seat+' seats selected</b> for '+r.guests+' guest(s)'+(seat>=r.guests?'<span class="ok"> · capacity OK</span>':'<span class="err"> · select more seats</span>')}
